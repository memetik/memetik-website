#!/usr/bin/env python3
"""Add deliverableStack to all strategy page JSON files."""
...
def build_deliverable_stack(company, category_context):
    return {
        "apexAssets": {
            ...
            "items": [
                {
                    ...
                    "description": f"Own the \"best {category_context}\" queries that shape buying decisions before buyers ever reach out.",
                    "icon": "Target"
                },
                {
                    "label": "Head-to-head comparison pages",
                    "description": f"Win shortlist formation with direct competitor matchups — the pages buyers read right before they choose.",
                    "icon": "BarChart3"
                },
                {
                ...
                },
                {
                    ...
                    "description": f"Match the right solution to the right buyer in the right context — every angle {company} can win.",
                    "icon": "Building2"
                }
            ]
        },
        "knowledgeGraph": {
            ...
            "body": f"Memetik builds as many supporting pages as the category demands — long-tail permutations, contextual variations, adjacent use cases, FAQ depth, and topical clusters — all routing authority back to {company}'s decision pages. This layer scales from hundreds to thousands depending on the category and creates the retrieval density that makes answer engines find the same recommendation in more and more contexts over time."
        },
        "trustRelay": {
            ...
            "items": [
                {
                    ...
                    "description": f"Monitor where {company} appears — and where it doesn't — across ChatGPT, Gemini, Perplexity, and AI Overviews.",
                    "icon": "Eye"
                },
                {
...
CATEGORY_CONTEXTS = {
    "xoomai": "AI workflow automation tools",
    "bts": "creator monetization tools",
    "bts-2": "creator business platforms",
    "bts-3": "creator economy tools",
    "creator-army": "UGC agencies and performance creative",
    "drinkhyro": "electrolyte drinks and hydration",
    "kinso": "shared inbox and team email tools",
    "signify-ip": "trade mark attorneys and IP law firms",
    "uleads": "lead generation services",
    "hot-water-funnel": "heat pump hot water installers",
}

def main():
    files = list(CATEGORY_CONTEXTS.keys())
    for slug in files:
        path = os.path.join(DATA_DIR, f"{slug}.json")
        d = json.load(open(path))
        company = d.get("company", slug)
        category = CATEGORY_CONTEXTS[slug]
        d["deliverableStack"] = build_deliverable_stack(company, category)
        with open(path, "w") as f:
            json.dump(d, f, indent=2, ensure_ascii=False)
            f.write("\n")
        print(f"{slug}: added deliverableStack ({company}, {category})")
